...
CC 中 Task System 和 TodoWrite **同时存在**，通过 `isTodoV2Enabled()` 切换（`utils/tasks.ts:133`）——非交互式会话（SDK）默认用 Task，交互式用 TodoWrite。环境变量 `TODO_V2` 可强制覆盖。Task 有 TodoWrite 没有的：文件锁并发保护、依赖强制执行、ownership、fs.watch 响应式监听、生命周期 hooks。
