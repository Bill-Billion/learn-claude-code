...
Drag-and-drop workflow builders. No-code "AI Agent" platforms. Prompt-chain orchestration libraries. They share the same delusion: that stringing LLM API calls together with if-else branches, node graphs, and hard-coded routing logic amounts to "building an agent."

It does not. They produce Rube Goldberg machines: over-engineered, brittle, procedural rule pipelines with an LLM wedged in as a glorified text-completion node. That is not an agent. It is a shell script with delusions of grandeur.

**Prompt-plumber "agents" are the fantasy of programmers who do not build models.** They try to brute-force a simulation of intelligence by piling up procedural logic: enormous rule trees, node graphs, and cascading prompt chains, then hope enough glue code will make autonomous behavior emerge. It will not. Agency cannot be engineered into existence. Agency is learned, not programmed.

Those systems are dead on arrival: brittle, unscalable, and incapable of meaningful generalization. They are GOFAI, Good Old-Fashioned AI, resurrected for the modern age: symbolic rule systems abandoned by the field decades ago, now returning under a coat of LLM paint. New packaging, same dead end.
...
This repository currently preserves two tutorial tracks:

- **Current main track: root-level `s01-s22`**
  The root-level `s01_*` through `s22_*` directories are the new primary version and the recommended reading path. Each chapter contains a complete narrative README, English and Japanese translations, runnable `code.py`, and any necessary diagrams.
- **Legacy transition track: `docs/`, `agents/`, and the current `web/`**
  These retain the old 12-lesson system temporarily for existing readers, old links, and the web platform transition.

New readers should start at the root-level `s01_agent_loop/` and continue through `s22_goal_loop/`. An old link or the current web platform probably leads to the legacy 12-lesson version. Chapter numbers do not correspond perfectly between the two tracks, so do not mix them.

### Legacy-to-Current Mapping

| Legacy 12-lesson version | New 20-lesson version | Topic |
|---|---|---|
...
The current web platform still reads the old 12-lesson content under `docs/`. Read the new 22-lesson track directly from root-level `s01-s22`.
...
learn-claude-code/
  s01_agent_loop/          # One folder per chapter
    README.md              #   Chinese source document (complete narrative)
  ...
  web/                     # Currently generated from legacy docs/
  tests/
```
