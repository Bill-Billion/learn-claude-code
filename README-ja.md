...
| [Learn Pi Agent](./learn-pi-agent/) | プロトコルとイベント駆動ランタイムを学びたい TypeScript 開発者 | Node.js 22.19+ + TypeScript | 13 | 英語、中国語、日本語 | s01 から OpenAI 互換 API へ接続 |
...
実行可能な章は `.env` のプロバイダー設定を読み込みます。コースガイドには、コース内容から生成する Web 学習画面の手順もあります。
...
[Learn Pi Agent](./learn-pi-agent/) は、小さな Pi 風ランタイムを 13 の累積的な TypeScript レッスンで組み立てます。s01 から実モデルが安全な読み取り専用ツールを呼び、その結果を次の応答に使います。後のレッスンでもこのループを保ちながら、プロバイダーイベント、ツールのライフサイクル、ターン状態、セッション、コンテキストリソース、拡張機能、実行モード、信頼判定、パッケージ解決を加えます。s13 では、同じ実プロバイダー経路で Harness 全体を統合します。
...
| レッスン | Harness に加える層 |
| --- | --- |
| s01-s03 | 実モデルとツールのループ、Tool Schema、正規化したプロバイダーイベント |
| s04-s06 | ライフサイクルイベントを伴うツール実行、フック、ターン状態 |
| s07-s09 | セッションツリー、コンテキストリソース、拡張機能ランタイム |
| s10-s12 | 実行モード、Project Trust、パッケージ解決 |
| s13 | 同じ実プロバイダー経路で動く統合済み Harness |

s01 以降の章コマンドは実際のプロバイダーを呼び出します。実行ごとに表現やツールの選択は変わることがあります。学習時には、モデルとツールのループ、イベントプロトコル、状態の契約を確認してください。
...
`OPENAI_MODEL` の既定値は `gpt-4o-mini`、`OPENAI_BASE_URL` の既定値は OpenAI 公式 API です。続けて `npm run s02` から `npm run s13` まで実行します。応答とツール呼び出しはモデルが選ぶため、実際の出力は例と異なる場合があります。
...
例は `.env` から `LANGCHAIN_MODEL` とプロバイダーの認証情報を読みます。既定の構成は OpenAI です。別の Embedding 実装を注入しない限り、s11-s13 も OpenAI Embeddings を使います。
...
1. コースガイドを読み、コードを変更する前に最初のレッスンを実行します。
2. レッスンディレクトリを順に進み、前のレッスンから追加された仕組みを確認します。
3. 章のエントリーポイントを実行し、出力される状態またはイベントを調べます。
4. ツールの追加、操作の拒否、セッションの分岐、コンテキストリソースの変更など、境界を一つ変更します。
5. 同じレッスンをもう一度実行してから、次の章の実装と比較します。

実行可能な例が、各コースの学習主線です。モデルの具体的な表現は実行ごとに変わるため、固定された Transcript ではなく、モデルとツールのループ、イベント、状態を比較してください。3 コースは別々の環境とロックファイルを使うため、リポジトリのルートに共通のインストールコマンドはありません。
...
- 実モデルの実行例には、有料のプロバイダーアカウントが必要な場合があります。
