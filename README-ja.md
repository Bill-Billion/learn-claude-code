...
「Agent」という言葉は、prompt plumbing industry 全体に乗っ取られました。

drag-and-drop workflow builder。no-code「AI Agent」platform。prompt-chain orchestration library。これらは同じ幻想を共有しています。LLM API call を if-else branch、node graph、hard-coded routing logic でつなげれば「Agent を作った」ことになるという幻想です。

違います。そこで作られるのは Rube Goldberg machine、つまり過剰設計された脆い procedural rule pipeline に、LLM を豪華な text-completion node として押し込んだものです。それは Agent ではありません。大げさな思い込みを持つ shell script です。

**Prompt plumber 型「Agent」は、モデルを作らない programmer の幻想です。** 巨大な rule tree、node graph、連鎖する prompt waterfall といった procedural logic を積み上げ、十分な glue code から autonomous behavior が自然発生することを祈ります。起きません。engineering で agency を coding することはできません。Agency は learned であり、programmed ではありません。

そのような system は誕生時点で死んでいます。脆く、scale せず、根本的な generalization ability を持ちません。数十年前に学界が捨てた symbolic rule system、GOFAI（Good Old-Fashioned AI）の現代的な復活です。LLM の塗装を施して戻ってきただけで、包装は新しくても同じ行き止まりです。
...
この repository には現在、2 つの tutorial track があります。

- **新しい main track: root の `s01-s22`**
  root の `s01_*` から `s22_*` は新しい primary version で、現在の推奨 reading path です。各章に完全な narrative README、英語と日本語の翻訳、実行可能な `code.py`、必要な diagram があります。
- **legacy transition track: `docs/`、`agents/`、現在の `web/`**
  既存読者、旧 link、Web platform の移行用に、旧 12 章 system を一時的に保持しています。

新しい読者は root の `s01_agent_loop/` から `s22_goal_loop/` まで進んでください。旧 link または現在の Web platform から入ると、旧 12 章版を見ている可能性があります。chapter number は両 track で完全には対応しないため、混在させないでください。

### Legacy-to-Current Mapping

| 旧 12 章版 | 新 20 章版 | Topic |
|---|---|---|
...
現在の Web platform はまだ `docs/` の旧 12 章 content を読みます。新しい 22 章は root の `s01-s22` を直接読んでください。
...
learn-claude-code/
  s01_agent_loop/          # chapter ごとに 1 folder
    README.md              #   中国語 source document（完全な narrative）
    README.en.md           #   英語訳
    README.ja.md           #   日本語訳
    code.py                #   単独で実行可能なコード
