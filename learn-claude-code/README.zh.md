...
Prompt Chain、编排库、状态图和工作流构建器都可以是有效的 Harness 工具。它们能明确控制流、持久化状态、路由工作、执行重试与审批，也能让可重复流程变得可观察、可恢复。

但它们本身不会创造 Agency。固定图可以约束并协调模型周围的执行；面对陌生情境时进行理解、推理并选择行动的能力，仍来自训练后的模型。问题不在于使用编排，而在于把编排结构误当成模型智能。

实际边界很简单：流程本应固定时使用确定性编排，下一步需要判断时交给模型。一个好的 Agent 产品往往会同时使用两者。Harness 组织环境与执行，模型提供通过训练获得的行动能力。
...
- **当前主线：本课程目录中的 `s01-s22`**
  `learn-claude-code/` 内的 `s01_*` 到 `s22_*` 是主版本，也是当前推荐阅读路径。每章包含三语完整叙事 README、可运行的 `code.py`，以及必要的图示。
- **旧版过渡：`docs/` 和 `agents/`**
  这些仍暂时保留旧 12 章体系，供已有读者和旧链接使用。

新读者请从 `learn-claude-code/s01_agent_loop/` 读到 `learn-claude-code/s22_goal_loop/`。旧版章节号和当前主线不完全一致，不要混用章节号。

### 旧版到新版的对应关系

| 旧 12 章版本 | 当前 22 章版本 | 主题 |
|---|---|---|
...
| 新版新增 | s03、s04、s09、s10、s11、s14、s19、s20、s21、s22 | Permission、Hooks、Memory、System Prompt、Error Recovery、Cron、MCP、Comprehensive Agent、Workflow Runtime、Goal Loop |
...
Web 平台会在构建时从本课程目录中的 `s01_*` 到 `s22_*` 提取当前 22 章内容。`npm run dev` 和 `npm run build` 都会自动执行这一步。
...
```
learn-agent-harness/
  learn-claude-code/       # 本课程目录
    s01_agent_loop/        # 每章一个文件夹
      README.md            #   英文课程正文（完整叙事）
      README.zh.md         #   中文译本
      README.ja.md         #   日文译本
      code.py              #   独立可运行代码
      images/              #   SVG 流程图
