...
export const VERSION_META: Record<string, {
  ...
  s21: {
    ...
    subtitle: "The Model Decides Each Step; a Script Decides the Orchestration",
    coreAddition: "Deterministic workflow runtime",
    keyInsight: "Fixed multi-agent orchestration belongs in recoverable scripts when the process shape is known.",
    layer: "collaboration",
    prevVersion: "s20",
  },
  s22: {
    title: "Goal Loop",
    subtitle: "The Goal Decides When to Stop, Not the Model",
    coreAddition: "Evidence-backed goal gate",
    keyInsight: "A turn should end only when trusted evidence satisfies the goal, not merely when the model stops.",
    layer: "planning",
    prevVersion: "s21",
  },
};
...
export const LAYERS = [
  ...
  },
  {
    ...
    color: "#EF4444",
    versions: ["s12", "s15", "s16", "s17", "s18", "s19", "s20", "s21"],
  },
] as const;
