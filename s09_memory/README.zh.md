...
提取的 prompt 里还带着已有记忆的清单，让模型只在"确实有新东西"时返回内容，避免同一条偏好写十遍。但模型给出的只是候选，不是写盘许可。每条候选都要带 `scope`：`persistent` 表示以后新会话仍然适用，`current_task` 则装本次任务的命令、临时路径和临时限制。

最后一道检查在 `should_store_memory()`。只有字段完整的 `scope="persistent"` 候选才能写入；带"本次会话"、"当前任务"一类临时字样的内容会被拒绝，名字、描述或正文与已有记忆重复的也不会再写。这样即使提取模型误判，"这次不要创建文件"也不会悄悄变成下周还在生效的规矩。
